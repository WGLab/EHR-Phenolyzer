...
parser.add_argument(
    "-d", "--outdir", dest="outdir", default="out", help="the path to the output folder"
)
parser.add_argument(
    "-k", "--keeptmp", dest="keeptmp", action="store_true", help="keep temporary files"
)
parser.add_argument(
    "-m",
    "--omim",
    dest="omim",
    default=os.path.join(os.path.dirname(__file__), "./db/OMIM_HGNCGenes.txt"),
...
parser.add_argument(
    ...
    "--obo",
    dest="obo",
    default=os.path.join(os.path.dirname(__file__), "./db/hp.obo"),
...
hpo_file_tmp1 = (
    args["outdir"] + "/" + args["prefix"] + ".hpo.txt"
)  # output from NLP processing
hpo_file_tmp2 = (
    args["outdir"] + "/" + args["prefix"] + ".hpo.tmp.txt"
)  # only with HPO names, which is used as Phenolyzer input
...
for line in open(hpo_file_tmp1):
    ...
    if (
        not sub_ontology[0] == "HP:0000118"
    ):  # only consider HPO ids that belong to Phenotypic abnormality
        print(
            "Notice: "
            + hpo_id
            + " "
            + hpo_name
            + " was ignored, because it doens't belong to Phenotypic abnormality HPO terms"
        )
        continue
...
phenolyzer_command = "disease_annotation.pl -f -p -ph -logistic -addon DB_DISGENET_GENE_DISEASE_SCORE,DB_GAD_GENE_DISEASE_SCORE -addon_weight 0.25 {0} -out {1}/{2}.tmp".format(
    os.path.abspath(hpo_file_tmp2), args["outdir"], args["prefix"]
)
...
outfile = open(
    args["outdir"] + "/" + args["prefix"] + ".EHRPhenolyzer.OMIMGenes.txt", "w"
)
outfile.write("rank\tgene\n")
i = 0
